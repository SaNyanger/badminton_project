...
class _MainPageState extends State<MainPage> {
  ...
  Widget build(BuildContext context) {
    print('[MainPage] build 호출됨');
    print('[MainPage] BaddyUser 등록 상태: ${Get.isRegistered<BaddyUser>()}');

    if (!Get.isRegistered<BaddyUser>()) {
      // AuthPage로 강제 리다이렉션
      Future.microtask(() {
        print('[MainPage] BaddyUser 없음 → AuthPage 이동');
        Get.offAll(() => const AuthPage());
      });
      return const Scaffold(body: Center(child: CircularProgressIndicator()));
    }
    final BaddyUser currentUser = Get.find<BaddyUser>();
